...
const Toolbar = ({
	...
	<MuiToolbar
		className={classnames({ [classes.mobileToolbar]: width === 'xs' }, { [classes.defaultToolbar]: width !== 'xs' }, className)}
		disableGutters
		{...rest}
	>
		{Children.count(children) === 0 ? (
			<div className={classes.defaultToolbar}>
				<SaveButton handleSubmitWithRedirect={handleSubmitWithRedirect} invalid={invalid} redirect={redirect} saving={saving} submitOnEnter={submitOnEnter} />
				{record && record.id && <DeleteButton basePath={basePath} record={record} resource={resource} />}
			</div>
		) : (
